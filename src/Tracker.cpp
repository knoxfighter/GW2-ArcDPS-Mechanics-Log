...
void Tracker::processMechanic(const cbtevent* ev, PlayerEntry* new_player_src, PlayerEntry* new_player_dst, Mechanic* new_mechanic, int64_t value)
{
	...
	if (new_mechanic->is_multihit && ev->time < (relevant_entry->getLastMechanicHitTime(new_mechanic->ids[0]) + new_mechanic->frequency_player)) // For double used: (new_mechanic->is_double_used ? relevant_entry->getLastMechanicHitTime( new_mechanic->name)  :
	{
		return;
	}
